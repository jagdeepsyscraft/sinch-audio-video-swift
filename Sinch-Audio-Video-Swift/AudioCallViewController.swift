...
enum AButtonsBar: Int {
    
    case decline
    case hangup
}

class AudioCallViewController:UIViewController,SINCallDelegate {
    ...
    {
        startCallDurationTimerWithSelector(#selector(AudioCallViewController.onDurationTimer(_:)))
        showButtons(.hangup)
...
// MARK: - This extension contains UI helper methods for AudioCallViewController
...
extension AudioCallViewController {
    ...
    func showButtons(_ buttons: AButtonsBar) {
        if buttons == .decline {
    ...
    func startCallDurationTimerWithSelector(_ selector: Selector) {
        ...
        durationTimer = Timer.scheduledTimer(timeInterval: 0.5, target: self, selector: #selector(AudioCallViewController.internal_updateDurartion(_:)), userInfo: selectorString, repeats: true)
    }
